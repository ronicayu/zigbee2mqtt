...
export default class Zigbee {
  ...
  async start(): Promise<"reset" | "resumed" | "restored"> {
    ...
    const herdsmanSettings = {
      network: {
        panID:
          settings.get().advanced.pan_id === "GENERATE"
            ? this.generatePanID()
            : (settings.get().advanced.pan_id as number),
        extendedPanID:
          settings.get().advanced.ext_pan_id === "GENERATE"
            ? this.generateExtPanID()
            : (settings.get().advanced.ext_pan_id as number[]),
    ...
    const herdsmanSettingsLog = objectAssignDeep({}, herdsmanSettings, {
      network: { networkKey: "HIDDEN" },
    });
    logger.debug(
      `Using zigbee-herdsman with settings: '${stringify(herdsmanSettingsLog)}'`
    );
    ...
    this.herdsman.on("adapterDisconnected", () =>
      this.eventBus.emitAdapterDisconnected()
    );
    this.herdsman.on(
      "lastSeenChanged",
      (data: ZHEvents.LastSeenChangedPayload) => {
        this.eventBus.emitLastSeenChanged({
          device: this.resolveDevice(data.device.ieeeAddr),
          reason: data.reason,
        });
      }
    );
    this.herdsman.on(
      "permitJoinChanged",
      (data: ZHEvents.PermitJoinChangedPayload) => {
        this.eventBus.emitPermitJoinChanged(data);
      }
    );
    this.herdsman.on(
      "deviceNetworkAddressChanged",
      (data: ZHEvents.DeviceNetworkAddressChangedPayload) => {
    ...
    this.herdsman.on(
      "deviceAnnounce",
      (data: ZHEvents.DeviceAnnouncePayload) => {
        const device = this.resolveDevice(data.device.ieeeAddr);
    ...
    this.herdsman.on(
      "deviceInterview",
      async (data: ZHEvents.DeviceInterviewPayload) => {
        const device = this.resolveDevice(data.device.ieeeAddr);
    ...
    this.herdsman.on(
      "deviceJoined",
      async (data: ZHEvents.DeviceJoinedPayload) => {
        const device = this.resolveDevice(data.device.ieeeAddr);
        /* istanbul ignore if */ if (!device) return; // Prevent potential race
        await device.resolveDefinition();
    ...
    this.herdsman.on("deviceLeave", (data: ZHEvents.DeviceLeavePayload) => {
      const name =
        settings.getDevice(data.ieeeAddr)?.friendly_name || data.ieeeAddr;
    ...
    this.herdsman.on("message", async (data: ZHEvents.MessagePayload) => {
      ...
      logger.debug(
        `Received Zigbee message from '${device.name}', type '${data.type}', ` +
          `cluster '${data.cluster}', data '${stringify(
            data.data
          )}' from endpoint ${data.endpoint.ID}` +
          (data.hasOwnProperty("groupID")
            ? ` with groupID ${data.groupID}`
            : ``) +
          (device.zh.type === "Coordinator"
            ? `, ignoring since it is from coordinator`
            : ``)
      );
    ...
    logger.info(
      `Coordinator firmware version: '${stringify(
        await this.getCoordinatorVersion()
      )}'`
    );
    logger.debug(
      `Zigbee network parameters: ${stringify(
        await this.herdsman.getNetworkParameters()
      )}`
    );
    ...
    for (const device of this.devices(false)) {
      ...
      const remove = async (device: Device): Promise<void> => {
        try {
          ...
          logger.error(
            `Failed to remove '${device.ieeeAddr}' (${error.message})`
          );
        }
      };
      if (passlist.length > 0) {
        if (!passlist.includes(device.ieeeAddr)) {
          logger.warn(
            `Device which is not on passlist connected (${device.ieeeAddr}), removing...`
          );
          await remove(device);
        }
      } else if (blocklist.includes(device.ieeeAddr)) {
        logger.warn(
          `Device on blocklist is connected (${device.ieeeAddr}), removing...`
        );
        await remove(device);
      }
    }
  ...
  async coordinatorCheck(): Promise<{ missingRouters: Device[] }> {
    ...
    return {
      missingRouters: check.missingRouters.map((d) =>
  ...
  resolveEntity(key: string | number | zh.Device): Device | Group {
    if (typeof key === "object") {
      ...
      return this.resolveDevice(
        this.herdsman.getDevicesByType("Coordinator")[0].ieeeAddr
      );
    } else {
  ...
  resolveEntityAndEndpoint(ID: string): {
    ...
    const endpoint = deviceOrGroup?.isDevice()
      ? deviceOrGroup.endpoint(endpointNameOrID)
      : null;

    return {
      ID: entityName,
      entity: deviceOrGroup,
      endpointID: endpointNameOrID,
      endpoint: endpoint,
    };
  }
  ...
  @bind private async acceptJoiningDeviceHandler(
    ieeeAddr: string
  ): Promise<boolean> {
    ...
    if (passlist.length > 0) {
      if (passlist.includes(ieeeAddr)) {
        logger.info(
          `Accepting joining device which is on passlist '${ieeeAddr}'`
        );
        return true;
      } else {
      ...
      if (blocklist.includes(ieeeAddr)) {
        logger.info(
          `Rejecting joining device which is on blocklist '${ieeeAddr}'`
        );
        return false;
      } else {
